...
namespace TivoProxy
{
    ...
    public class TivoProxyEventSource : EventSource
    {
        ...
        public void ClientConnected(TivoMode mode, string clientAddress, string streamProperties)
        {
            WriteEvent(1, mode, clientAddress, streamProperties);
        }

        [NonEvent]
        public void ClientConnectionFailure(TivoMode mode, Exception exception)
        {
            this.ClientConnectionFailure(mode, exception.ToString());
        }

        [Event(2, Message = "Failure authenticating client connection [{0}] :\n{1}")]
        public void ClientConnectionFailure(TivoMode mode, string exceptionText)
        {
            WriteEvent(2, mode, exceptionText);
        }

        [Event(3, Message = "Server connected [{0}] : Server = {1}")]
        public void ServerConnected(TivoMode mode, string serverAddress)
        {
            WriteEvent(3, mode, serverAddress);
        }

        [NonEvent]
        public void ServerConnectionFailure(TivoMode mode, Exception exception)
        {
            this.ServerConnectionFailure(mode, exception.ToString());
        }

        [Event(4, Message = "Failure authenticating server connection [{0}] :\n{1}")]
        public void ServerConnectionFailure(TivoMode mode, string exceptionText)
        {
            WriteEvent(4, mode, exceptionText);
        }

        [Event(5, Message = "SENT -> {0} : {2} - {3}\n{4}\n")]
        public void MessageFromClient(TivoMode mode, int schemaVersion, int rpcId, string type, string message)
        {
            WriteEvent(5, mode, schemaVersion, rpcId, type, message);
        }

        [Event(6, Message = "{0} -> RECV : {2} - {3}\n{4}\n")]
        public void MessageFromServer(TivoMode mode, int schemaVersion, int rpcId, string type, string message)
        {
