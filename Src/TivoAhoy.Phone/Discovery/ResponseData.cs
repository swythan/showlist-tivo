namespace TivoAhoy.Phone.Discovery
{
    ...
    public class Txt : ResponseData
    {
        ...
        public void AddProperty(string txt)
        {
            ...
            if (kvp.Length == 2)
            {
                if (string.Compare(kvp[1], True, StringComparison.InvariantCultureIgnoreCase) == 0)
                    Properties.Add(kvp[0], True);
                else if (string.Compare(kvp[1], False, StringComparison.InvariantCultureIgnoreCase) == 0)
